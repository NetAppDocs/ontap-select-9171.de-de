...
[role="lead"]
ONTAP Select basiert auf ONTAP Storage-Software und bietet effiziente Enterprise-Speicherdienste mit einer hochverfügbaren, Shared-Nothing Scale-Out-Architektur. Eine Lösung kann mit einem, zwei, vier, sechs oder acht Knoten mit bis zu 400TB Bruttokapazität für NFS-, SMB/CIFS- und iSCSI-verbundenen Speicher pro Knoten bereitgestellt werden. Die native Deduplizierung und Komprimierung ermöglicht eine Senkung der Speicherkosten durch Erhöhung der effektiven Kapazität. Die Scale-Out-Architektur ermöglicht hohe Verfügbarkeit und unterbrechungsfreien Datenzugriff für Load Balancing oder zur Wartung von Hardware.
